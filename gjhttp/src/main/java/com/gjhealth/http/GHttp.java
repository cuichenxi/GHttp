...
package com.gjhealth.http;

import android.app.Application;
import android.content.Context;
import android.text.TextUtils;

import com.gjhealth.http.cache.RxCache;
import com.gjhealth.http.cache.converter.IDiskConverter;
import com.gjhealth.http.cache.converter.SerializableDiskConverter;
import com.gjhealth.http.cache.model.CacheMode;
import com.gjhealth.http.cookie.CookieManger;
import com.gjhealth.http.https.HttpsUtils;
import com.gjhealth.http.interceptor.HttpLoggingInterceptor;
import com.gjhealth.http.model.HttpHeaders;
import com.gjhealth.http.model.HttpParams;
import com.gjhealth.http.request.CustomRequest;
import com.gjhealth.http.request.DeleteRequest;
import com.gjhealth.http.request.DownloadRequest;
import com.gjhealth.http.request.GetRequest;
import com.gjhealth.http.request.PostRequest;
import com.gjhealth.http.request.PutRequest;
import com.gjhealth.http.utils.HttpLog;
import com.gjhealth.http.utils.RxUtil;
import com.gjhealth.http.utils.Utils;
...
public final class GHttp {
    ...
    private volatile static GHttp singleton = null;
    ...
    private static void testInitialize() {
        if (sContext == null)
            throw new ExceptionInInitializerError("请先在全局Application中调用 GHttp.init() 初始化！");
    }
     ...
     */
    public GHttp debug(String tag, boolean isPrintException) {
     ...
     * https的全局访问规则
     */
    public GHttp setHostnameVerifier(HostnameVerifier hostnameVerifier) {
     ...
     * https的全局自签名证书
     */
    public GHttp setCertificates(InputStream... certificates) {
     ...
     * https双向认证证书
     */
    public GHttp setCertificates(InputStream bksFile, String password, InputStream... certificates) {
     ...
     * 全局cookie存取规则
     */
    public GHttp setCookieStore(CookieManger cookieManager) {
     ...
     * 全局读取超时时间
     */
    public GHttp setReadTimeOut(long readTimeOut) {
     ...
     * 全局写入超时时间
     */
    public GHttp setWriteTimeOut(long writeTimeout) {
     ...
     * 全局连接超时时间
     */
    public GHttp setConnectTimeout(long connectTimeout) {
     ...
     * 超时重试次数
     */
    public GHttp setRetryCount(int retryCount) {
     ...
     * 超时重试延迟时间
     */
    public GHttp setRetryDelay(int retryDelay) {
     ...
     * 超时重试延迟叠加时间
     */
    public GHttp setRetryIncreaseDelay(int retryIncreaseDelay) {
     ...
     * 全局的缓存模式
     */
    public GHttp setCacheMode(CacheMode cacheMode) {
     ...
     * 全局的缓存过期时间
     */
    public GHttp setCacheTime(long cacheTime) {
     ...
     * 全局的缓存大小,默认50M
     */
    public GHttp setCacheMaxSize(long maxSize) {
     ...
     */
    public GHttp setCacheVersion(int cacheersion) {
        if (cacheersion < 0)
     ...
     */
    public GHttp setCacheDirectory(File directory) {
     ...
     * 全局设置缓存的转换器
     */
    public GHttp setCacheDiskConverter(IDiskConverter converter) {
     ...
     * 全局设置OkHttp的缓存,默认是3天
     */
    public GHttp setHttpCache(Cache cache) {
        this.mCache = cache;
        return this;
    }
     ...
     * 添加全局公共请求参数
     */
    public GHttp addCommonParams(HttpParams commonParams) {
     ...
     * 添加全局公共请求参数
     */
    public GHttp addCommonHeaders(HttpHeaders commonHeaders) {
     ...
     * 添加全局拦截器
     */
    public GHttp addInterceptor(Interceptor interceptor) {
     ...
     * 添加全局网络拦截器
     */
    public GHttp addNetworkInterceptor(Interceptor interceptor) {
     ...
     * 全局设置代理
     */
    public GHttp setOkproxy(Proxy proxy) {
     ...
     * 全局设置请求的连接池
     */
    public GHttp setOkconnectionPool(ConnectionPool connectionPool) {
     ...
     */
    public GHttp setOkclient(OkHttpClient client) {
     ...
     */
    public GHttp addConverterFactory(Converter.Factory factory) {
     ...
     */
    public GHttp addCallAdapterFactory(CallAdapter.Factory factory) {
     ...
     */
    public GHttp setCallbackExecutor(Executor executor) {
     ...
     */
    public GHttp setCallFactory(okhttp3.Call.Factory factory) {
     ...
     * 全局设置baseurl
     */
    public GHttp setBaseUrl(String baseUrl) {
