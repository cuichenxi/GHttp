...
package com.gjhealth.http.request;

import android.content.Context;
import android.text.TextUtils;

import com.gjhealth.http.GHttp;
import com.gjhealth.http.api.ApiService;
...
import com.gjhealth.http.interceptor.BaseDynamicInterceptor;
import com.gjhealth.http.interceptor.CacheInterceptor;
import com.gjhealth.http.interceptor.CacheInterceptorOffline;
import com.gjhealth.http.interceptor.HeadersInterceptor;
import com.gjhealth.http.interceptor.NoCacheInterceptor;
...
import static com.gjhealth.http.GHttp.getRetrofitBuilder;
import static com.gjhealth.http.GHttp.getRxCache;
...
public abstract class BaseRequest<R extends BaseRequest> {
    ...
    public BaseRequest(String url) {
        this.url = url;
        context = GHttp.getContext();
        GHttp config = GHttp.getInstance();
    ...
    public R cacheTime(long cacheTime) {
        if (cacheTime <= -1) cacheTime = GHttp.DEFAULT_CACHE_NEVER_EXPIRE;
    ...
    private OkHttpClient.Builder generateOkClient() {
        if (readTimeOut <= 0 && writeTimeOut <= 0 && connectTimeout <= 0 && sslParams == null
            ...
            OkHttpClient.Builder builder = GHttp.getOkHttpClientBuilder();
            ...
            final OkHttpClient.Builder newClientBuilder = GHttp.getOkHttpClient().newBuilder();
            if (readTimeOut > 0)
            ...
            if (cookies.size() > 0) GHttp.getCookieJar().addCookies(cookies);
    ...
    private Retrofit.Builder generateRetrofit() {
        if (converterFactories.isEmpty() && adapterFactories.isEmpty()) {
            ...
            if (!converterFactories.isEmpty()) {
                ...
                //获取全局的对象重新设置
                Retrofit.Builder newBuilder = GHttp.getRetrofitBuilder();
    ...
    private RxCache.Builder generateRxCache() {
        final RxCache.Builder rxCacheBuilder = GHttp.getRxCacheBuilder();
        switch (cacheMode) {
            case NO_CACHE://不使用缓存
            ...
            case DEFAULT://使用Okhttp的缓存
                if (this.cache == null) {
                    File cacheDirectory = GHttp.getCacheDirectory();
                    if (cacheDirectory == null) {
                        cacheDirectory = new File(GHttp.getContext().getCacheDir(), "okhttp-cache");
                    } else {
                        if (cacheDirectory.isDirectory() && !cacheDirectory.exists()) {
                            cacheDirectory.mkdirs();
                        }
                    }
                    this.cache = new Cache(cacheDirectory, Math.max(5 * 1024 * 1024, GHttp.getCacheMaxSize()));
                }
                String cacheControlValue = String.format("max-age=%d", Math.max(-1, cacheTime));
                final CacheInterceptor REWRITE_CACHE_CONTROL_INTERCEPTOR = new CacheInterceptor(GHttp.getContext(), cacheControlValue);
                final CacheInterceptorOffline REWRITE_CACHE_CONTROL_INTERCEPTOR_OFFLINE = new CacheInterceptorOffline(GHttp.getContext(), cacheControlValue);
